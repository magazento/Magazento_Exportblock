...
class Magazento_Exportblock_Model_Export
{
    private $status = null;
    ...
    public function loadManualCollection($itemIds) {
        ...
        $collection->addFieldToFilter('block_id', array('in' => $itemIds));
        return $collection;
    }
    ...
    public function loadCollection($profile) {
        ...
        $collection->getSelect()->where('block_store.store_id IN (?)', 0);
    ...
    public function exportItemsForProfile($profileId) {
        $profile = Mage::getModel('exportblock/item')->load($profileId);
